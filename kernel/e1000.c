...
e1000_transmit(struct mbuf *m)
{
  ...
  //
  //printf("e1000: transmission started\n");
  acquire(&e1000_lock);
  ...
  //printf("e1000: transmission done\n");
  return 0;
}
...
e1000_recv(void)
{
  //printf("e1000: receving started\n");
  acquire(&e1000_lock);
  uint64 start = (regs[E1000_RDT] + 1) % RX_RING_SIZE;
  for (uint64 i = start; rx_ring[i].status & E1000_RXD_STAT_DD; i = (i + 1) % RX_RING_SIZE)
  {
     struct rx_desc* desc = &rx_ring[i];
     struct mbuf* buf = rx_mbufs[i];
